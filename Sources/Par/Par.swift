...
public class Par {
    ...
    func parseNode(_ superNode: ParNode, _ parItem: ParItem, _ level: Int) -> ParNode? {
        ...
        func addSub(_ pattern: String, _ any: ParItem) {
            let parNode = ParNode(pattern)
            if let nextNode = parseNode(parNode, any, level+1) {
                lastNode = nextNode
                let _ = ParEdge(superNode, nextNode)
            }
        }
        ...
        for nexti in parItem.nextPars {
            ...
            if Par.trace2 { print (nexti.node.pattern, terminator: " ") }
            let pattern =  nexti.node.pattern
            switch pattern {
